...
router.post("/", auth, async (req, res) => {
  try {
    ...
    if (book.rows[0]?.status !== "dostepna") return res.status(400).json({ message: "Niedostępna" });

    await pool.query("BEGIN");
    await pool.query("INSERT INTO wypozyczenia (id_ksiazki, id_uzytkownika) VALUES ($1, $2)", [id_ksiazki, id_uzytkownika]);
...
router.post("/return", auth, async (req, res) => {
  try {
    const { id_wypozyczenia, id_ksiazki } = req.body;
    await pool.query("BEGIN");
    await pool.query("UPDATE wypozyczenia SET status_wypozyczenia = 'oddana', data_zwrotu = NOW() WHERE id_wypozyczenia = $1", [id_wypozyczenia]);
    await pool.query("UPDATE ksiazki SET status = 'dostepna' WHERE id_ksiazki = $1", [id_ksiazki]);
    await pool.query("COMMIT");
